...
function isCreditCardNumber(ccn) {
  ...
  const sum = arr.reduce((prev, curr, index) => {
    if (index % 2) return prev + curr;
    return prev + (curr * 2 > 9 ? curr * 2 - 9 : curr * 2);
  }, 0);
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(num.toString().split('').map((item) => parseInt(item, 10)).reduce((a, b) => a + b));
}
...
function isBracketsBalanced(str1) {
  ...
  while (str.length > 0 && length > str.length) {
    length = str.length;
    str = str.replace(/\[\]|\{\}|\(\)|<>/g, '');
  }
  return str.length === 0;
...
function evaluateTicTacToePosition(p) {
  ...
  return winLines.map((line) => line.reduce((prev, curr) => prev + curr), '').map((line) => {
